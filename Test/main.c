...
//可以让节点在配网完成后，通过发布onoff状态消息来控制快速配网中的其他节点。 发布使用APPKEY 2进行加密,所以只有同一个APPKEY 2的节点可以解析。
//参数1：该发布的名称，发布消息使用的APPKEY索引值，长度，ROLE_FAST_PROV:发布的目标角色是fast provisioning process。
ESP_BLE_MESH_MODEL_PUB_DEFINE(onoff_pub, 2 + 3, ROLE_FAST_PROV);

//该代码主要是完成了开关模型运行所需结构体变量的定义和初始化,实现消息自动响应功能
static esp_ble_mesh_gen_onoff_srv_t onoff_server = {
    .rsp_ctrl.get_auto_rsp = ESP_BLE_MESH_SERVER_AUTO_RSP, //表示开关量获取消息会自动回复。
    .rsp_ctrl.set_auto_rsp = ESP_BLE_MESH_SERVER_AUTO_RSP, //开关量设置消息也会自动回复。
};

//定义并注册了快配网模型所支持的标准操作类型,为快速配网消息处理打下基础。这对整个Mesh网络配网流程是必需的。
static esp_ble_mesh_model_op_t fast_prov_srv_op[] = {
    // 设置快配信息,payload最大3字节
    ESP_BLE_MESH_MODEL_OP(ESP_BLE_MESH_VND_MODEL_OP_FAST_PROV_INFO_SET,          3),
    // 添加网络密钥,payload最大16字节
...
/* 解耦的概念
解耦在软件架构设计中是一个非常重要的概念,主要意思是使软件系统的不同组件或模块之间相互独立,减少彼此的依赖。
其目的是提高系统的灵活性、可维护性和可重用性。
*/

//初始化了两个vendor模型,这样的模型设计使得快速配网的实现逻辑根应用层解耦，通过模型层进行交互。
//ESP_BLE_MESH_VENDOR_MODEL(_company, _id, _op, _pub, _user_data)
static esp_ble_mesh_model_t vnd_models[] = {
    //第一个模式是 fast provisioning server模型，快速配网服务器模型
    ESP_BLE_MESH_VENDOR_MODEL(CID_ESP, ESP_BLE_MESH_VND_MODEL_ID_FAST_PROV_SRV,
    fast_prov_srv_op, NULL, &fast_prov_server),
    //第二个模型是 fast provisioning client模型,快速配网客户端模型
        ...
        esp_ble_mesh_prov_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_PROVISIONER_PROV_COMPLETE_EVT:
        ...
        provisioner_prov_complete(param->provisioner_prov_complete.node_idx,
                ...
                printf("param->provisioner_prov_complete.node_idx:%d\n ",param->provisioner_prov_complete.node_idx);
        break;
...
static esp_err_t ble_mesh_init(void)
{
    ...
    //注册配置服务器模型的回调函数，会处理配置服务器接收到的消息
    esp_ble_mesh_register_generic_server_callback(example_ble_mesh_generic_server_cb);
    // 注册通用服务器模型的回调函数，这里是onoff server回调函数会处理onooff状态变化事件

    // 初始化 BLE Mesh 协议栈
    err = esp_ble_mesh_init(&prov, &comp);
    esp_ble_mesh_set_unprovisioned_device_name("FAST_S_0");
    if (err != ESP_OK) {
    ...
    printf("test1.0\n");
    board_led_operation(LED_B, LED_ON); //配网成功改变指示灯状态
